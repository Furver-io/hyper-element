...
#### Using Attributes to Drive Conditionals

Component attributes can directly control what gets rendered:
...
// <notification-banner type="success" message="Saved successfully!" />
```
...
### Parent Listening for Custom Events
...
// Usage: <user-profile user-id="123"></user-profile>
```

For components that need to refetch when attributes change, trigger the fetch in both `setup()` and watch for attribute updates:
...
## Dual Reactivity: Attributes + Store
...
// 2. Store update: appStore.setUserStatus('123', 'online')
```

## Persisting State to Stores

Write component actions back to external stores (Redux pattern):
...
// <product-card name="Widget" price="9.99"></product-card>
```
